  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Zuletzt angesehene Profile</p>
      </div>
        ...
        <>
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">
              {historySeekers.length}{" "}
              {historySeekers.length === 1 ? "Profil" : "Profile"}
            </p>
          </div>

          <div className="space-y-4">
            {historySeekers.map((seeker: any) => (
              <div
                key={`${seeker.id}-${seeker.viewedAt}`}
                className="flex flex-col sm:flex-row gap-4 p-4 bg-white rounded-xl border border-gray-200"
              >
